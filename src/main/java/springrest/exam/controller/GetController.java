...
public class GetController {
    ...
    @GetMapping(value = "/var1/{variable}")//@PathVariable: requestPath로 받은 것
    ...
    @GetMapping(value = "/req1")//매개변수 여러개 설정
    public String getRequestParam1(String name, String email, String phone) {
        return name + " " + email + " " + phone;
    }

    @GetMapping(value = "/req2")//전달받을 쿼리 여러개일 때 Map객체로 전달받는 방법
    public String getRequestParam2(@RequestParam Map<String, String> param) {
        StringBuilder sb = new StringBuilder();
        param.entrySet().forEach(map -> {
            sb.append(map.getKey() + " : " + map.getValue() + "\n");//map객체를 담을 때 각각의 key value 추출해서 담아 append함
        });
        return sb.toString();
    }

    @GetMapping(value = "/req3")//DTO로 전달받는 방법
    public String getRequestParam3(MemberDTO memberDTO) {
        return memberDTO.toString();
    }
//memberDTO 받아서 문자열로 리턴

    @GetMapping(value = "/req4")
    public MemberDTO getRequestParam4(MemberDTO memberDTO) {
        return memberDTO;
    }
//DTO객체 리턴(알아서 json으로 바꿔서 리턴)

    @GetMapping(value = "/req5")
    public ResponseEntity getRequestParam5(String name) {
        MemberDTO dto = new MemberDTO();//알아서 멤버 DTO객체 생성
        dto.setName(name);
        dto.setEmail("aaa@naver.com");
        dto.setPhone("010-3333-4444");

        HttpHeaders header = new HttpHeaders();//HttpHeaders 생성해서 응답헤더 생성
        header.setContentType(new MediaType("application", "json", Charset.forName("UTF-8")));

        return new ResponseEntity<>(dto, header, HttpStatus.OK);//ResponseEntity:바디, 헤더, 응답코드 리턴
    }
